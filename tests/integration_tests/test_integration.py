...
class SandboxIntegrationTests:
    """Stable local copy of the standard sandbox backend test suite."""
    ...
    def test_upload_binary_content_roundtrip(
        ...
        """Upload and download binary bytes without corruption."""
        if not self.has_sync:
            pytest.skip("Sync tests not supported.")
    ...
    def test_download_error_file_not_found(
        ...
        """Downloading a missing file should return file_not_found."""
        if not self.has_sync:
            pytest.skip("Sync tests not supported.")
    ...
    def test_download_error_invalid_path_relative(
        ...
        """Downloading a relative path should fail with invalid_path."""
        if not self.has_sync:
            pytest.skip("Sync tests not supported.")
        responses = sandbox_backend.download_files(["relative/path.txt"])
        assert responses == [
            FileDownloadResponse(
                path="relative/path.txt", content=None, error="invalid_path"
            )
        ]
    ...
    def test_upload_relative_path_returns_invalid_path(
        ...
        """Uploading to a relative path should fail with invalid_path."""
        if not self.has_sync:
            pytest.skip("Sync tests not supported.")
